...
public class Player extends Character{
    ...
    public void autoMove(Random random, GameEntry gameEntry){
        ...
        if (movingNextRoom && sabotageList.size > 0) {
            ...
            moveBy(-(float) Math.cos(movementAngle) * 10f, -(float) Math.sin(movementAngle) * 10f);
            ...
            if (!NPCChosen || chosenNPC.getCurrentScreen() != getCurrentScreen()) {
                ...
                if (getCurrentScreen().nonHostiles.size > 0 && (randomChance < 8|| getCurrentScreen().hostiles.size == 0)) {
